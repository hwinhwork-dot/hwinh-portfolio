...
/* FIX: Distinguish EN (primary) from VI (secondary) */
.lo-btn{
...
.lo-btn.lo-primary:hover{background:var(--accent2);border-color:var(--accent3);box-shadow:0 14px 36px rgba(124,106,247,.5);transform:translateY(-5px);}
...
function renderProjects() {
  ...
  // They are initialized in go('projects', ...) after the view becomes visible.
}

// ================================================================
// CHART FUNCTIONS — FIX: destroy instances before recreating
// ================================================================
    ...
    options:{
      ...
      plugins:{
        ...
        tooltip:{backgroundColor:'#252538',titleFont:{family:"'DM Mono',monospace"},bodyFont:{family:"'DM Sans',sans-serif"},borderColor:'rgba(255,255,255,.1)',borderWidth:1,
          callbacks:{label:c=>` ${c.dataset.label}: ${c.raw}${c.dataIndex===2?'% '+(isVI?'giảm':'reduction'):'%'}`}
        }
      }
    }
  });
...
function updateProjectChartLabels() {
  const isVI = lang === 'vi';
  const klt = document.getElementById('kpi-chart-title');
  ...
  if (klt) klt.textContent = isVI ? 'Phân tích KPI Kỹ thuật — So với AAC Truyền thống' : 'Technical KPI Breakdown — vs Traditional AAC';
  if (mlt) mlt.textContent = isVI ? 'So sánh Mô hình: LSTM V1 vs Transformer V2' : 'Model Comparison: LSTM V1 vs Transformer V2';
  if (roleLbl) roleLbl.textContent = isVI ? 'Vai trò' : 'Role';
  if (teamLbl) teamLbl.textContent = isVI ? 'Nhóm' : 'Team';
  if (advLbl) advLbl.textContent = isVI ? 'GVHD' : 'Advisor';
  if (sprintLbl) sprintLbl.textContent = isVI ? 'Burndown Sprints' : 'Sprint Burndown';
}
