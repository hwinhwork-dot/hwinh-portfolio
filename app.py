...
.lo-btn.lo-primary{background:var(--accent);border-color:var(--accent2);color:white;box-shadow:0 8px 22px rgba(124,106,247,.35);}
.lo-btn.lo-primary:hover{background:var(--accent2);border-color:var(--accent3);transform:translateY(-5px);}
...
<div id="lang-overlay">
  ...
  <div class="lo-options">
    <button class="lo-btn lo-primary" onclick="selectLang('en')">English<span>For Global Recruiters</span></button>
    <button class="lo-btn" onclick="selectLang('vi')">Tiếng Việt<span>Dành cho NTD trong nước</span></button>
  </div>
...
vi: {
  ...
  ux: {
    ...
    phases: [
      { icon: '🔍', num: '01', name: 'Thấu Cảm', desc: 'Nghiên cứu người dùng, phỏng vấn, quan sát hành vi để khám phá pain points ẩn' },
      { icon: '🎯', num: '02', name: 'Định Nghĩa', desc: 'Framing vấn đề, câu hỏi HMW, xác định nguyên nhân gốc từ dữ liệu' },
      { icon: '💡', num: '03', name: 'Lên Ý tưởng', desc: 'Brainstorm, user story mapping, ưu tiên hóa theo khung MoSCoW' },
      { icon: '📐', num: '04', name: 'Tạo Nguyên mẫu', desc: 'Wireframes, luồng low-fi, journey maps, Figma interactive mockups' },
      { icon: '🧪', num: '05', name: 'Kiểm Thử', desc: 'A/B testing, Interleaving, đánh giá heuristic UX' },
      { icon: '📈', num: '06', name: 'Đo Tác Động', desc: 'NPS tracking, chỉ số chuyển đổi, phân tích giữ chân, KPI kinh doanh' }
    ],
    cases: [
      {
        ...
        problem: `Các startup tham gia chương trình ươm tạo của SIHUB thường bỏ dở hành trình onboarding kỹ thuật số. <strong>Chỉ ~40% hoàn tất luồng kích hoạt</strong> — ảnh hưởng trực tiếp đến mức độ tham gia chương trình, sự hài lòng và điểm NPS. Đội ngũ đang xem NPS như một con số, không hiểu được vấn đề nằm ở đâu trong hành trình.`,
        approachTitle: 'Cách Tiếp Cận',
        steps: [
          { n:'01', t:'Audit Dữ liệu Hành vi', d:'Thu thập dữ liệu đa kênh để lập bản đồ hành trình thực. Xác định 3 điểm thoát nhiều nhất tại bước upload tài liệu và xác minh.' },
          { n:'02', t:'Workshop Journey Mapping', d:'Cùng staff SIHUB và 5 founder phỏng vấn để vẽ journey map end-to-end. Ghi lại cảm xúc, kỳ vọng và pain points từng bước.' },
          { n:'03', t:'Phân tích Nguyên nhân Gốc', d:'Áp dụng 5 Whys — vấn đề cốt lõi không phải form mà là yêu cầu tài liệu không rõ ràng khiến founder bỏ cuộc và không quay lại.' },
          { n:'04', t:'Thiết kế A/B Testing', d:'Variant A (hướng dẫn inline + progressive disclosure) vs Control (form gốc nhiều bước). Dùng Interleaving để so sánh công bằng.' }
        ],
        insightLabel: 'Insight Cốt Lõi',
        insight: `Vấn đề thực sự không phải công nghệ — mà là <strong>nút thắt thông tin tại bước 3 (yêu cầu tài liệu)</strong>. Founder không biết cần format gì, thử một lần bị từ chối, và không bao giờ quay lại. Một thay đổi UI nhỏ về hướng dẫn inline giải quyết 60% tỷ lệ bỏ cuộc.`,
        ...
        learnings: [
          { icon: '🔬', title: 'Research Trước', text: 'Dữ liệu hành vi tiết lộ điểm thoát thực sự, không phải nơi stakeholders giả định.' },
          { icon: '⚗️', title: 'Kiểm Thử Trước Khi Deploy', text: 'A/B testing ngăn một cuộc redesign toàn diện — thay đổi nhỏ có tác động lớn.' },
          { icon: '📊', title: 'NPS là Hệ thống', text: 'NPS như hệ thống tín hiệu, không phải điểm số — mỗi bước có NPS driver riêng.' }
        ]
      },
      {
        ...
        problemLabel: 'Vấn đề Đặt ra',
        problem: `Mô hình EchoMind đạt độ chính xác 92–95%, nhưng <strong>bác sĩ không tin tưởng nó</strong>. Trong môi trường lâm sàng, hệ thống hộp đen nói "bệnh nhân muốn uống nước" mà không giải thích TẠI SAO là một rủi ro, không phải công cụ. Thách thức: thiết kế UI khiến quyết định AI có thể giải thích được mà không làm quá tải bác sĩ.`,
        approachTitle: 'Cách Tiếp Cận',
        steps: [
          { n:'01', t:'Phân tích Heuristic HCI', d:'Áp dụng 10 Heuristics của Nielsen vào prototype Gradio ban đầu. Phát hiện lỗi nghiêm trọng: không có visibility of system status, không có error recovery path.' },
          { n:'02', t:'Lập bản đồ Cognitive Load', d:'Lập bản đồ tải nhận thức của bác sĩ thần kinh khi xem xét kết quả AI. Xác định 3 thời điểm tải cao cần attention heatmap bắt buộc.' },
          { n:'03', t:'Thiết kế Attention Map UI', d:'Thiết kế hệ thống overlay nhiều lớp — vùng xanh/vàng/đỏ trên dạng sóng EEG hiển thị vùng não nào mô hình chú ý khi giải mã từng từ.' },
          { n:'04', t:'Progressive Disclosure', d:'Áp dụng progressive disclosure: kết quả cơ bản (văn bản + %) → mở rộng attention map → mở rộng phân tích tín hiệu đầy đủ.' }
        ],
        insightLabel: 'Insight Cốt Lõi',
        insight: `Bác sĩ không hỏi "độ chính xác có cao không?" — họ hỏi <strong>"tôi có thể giải thích điều này cho gia đình bệnh nhân không?"</strong> Explainability không phải yêu cầu kỹ thuật — đó là yêu cầu về tin tưởng và trách nhiệm pháp lý. Dashboard cần trả lời câu hỏi đó.`,
        ...
        learnings: [
          { icon: '🏥', title: 'Ngữ cảnh Domain', text: 'UX y tế cần kiến trúc tin tưởng, không chỉ usability. Explainability là tính năng cốt lõi.' },
          { icon: '🧩', title: 'Progressive Disclosure', text: 'Hiển thị mọi thứ cùng lúc làm bác sĩ choáng ngợp. Reveal theo lớp phù hợp workflow lâm sàng.' },
          { icon: '🎨', title: 'Heuristics là Scaffold', text: '10 Heuristics của Nielsen là khung audit có cấu trúc, phát hiện 6 vấn đề nghiêm trọng nhóm bỏ sót.' }
        ]
      },
      {
        ...
        problemLabel: 'Vấn đề Đặt ra',
        problem: `Học sinh nhận thiết bị e-reading <strong>bỏ dở quá trình cài đặt trước khi sử dụng lần đầu</strong>. Hành trình cài đặt có 7 bước với nhiều form, đăng ký tài khoản và đồng bộ nội dung — gây cognitive overload nghiêm trọng ở bước 2–3. Kết quả: thiết bị đắt tiền nằm trong cặp sách không được dùng.`,
        approachTitle: 'Cách Tiếp Cận',
        steps: [
          { n:'01', t:'Audit Cognitive Load', d:"Đo số quyết định mỗi bước trong luồng 7-bước. Bước 2 và 3 có 5+ quyết định đồng thời — vượt giới hạn bộ nhớ làm việc 3–4 mục (Định luật Miller)." },
          { n:'02', t:'Áp dụng Nguyên lý HCI', d:"Áp dụng Định luật Fitts (kích thước target touch), Định luật Hick (giảm lựa chọn mỗi bước), và Gestalt proximity để tái thiết kế từng giai đoạn." },
          { n:'03', t:'Journey Map — Học sinh Lớp 6', d:"Lập journey map đầy đủ: từ mở hộp → buổi đọc kỹ thuật số đầu tiên. Ghi lại cảm xúc, rào cản, kỳ vọng và nhu cầu hỗ trợ từng bước." },
          { n:'04', t:'Tái thiết kế Luồng 7 → 3 Bước', d:"Chunk 7 bước thành 3 giai đoạn: Kích hoạt Thiết bị → Cài đặt Nội dung → Cá nhân hóa. Defer các bước không quan trọng sang sau lần dùng đầu." }
        ],
        insightLabel: 'Insight Cốt Lõi',
        insight: `Học sinh không thất bại vì thiếu kỹ năng công nghệ — mà vì hệ thống <strong>vi phạm nguyên tắc "một hành động chính mỗi màn hình"</strong>. Bước 2 yêu cầu nhập mã trường, số điện thoại phụ huynh, lớp học và ngôn ngữ cùng lúc. Chia nhỏ thành các micro-steps tuần tự giảm độ phức tạp nhận thức ~60%.`,
        ...
        learnings: [
          { icon: '🧠', title: "Định luật Miller", text: "Chunk thông tin ≤4 mục mỗi quyết định là nguyên tắc HCI tác động nhất với người dùng không chuyên." },
          { icon: '📱', title: 'Một Hành động/Màn hình', text: 'Progressive disclosure — defer các bước không quan trọng — giảm phức tạp nhận thức mà không mất chức năng.' },
          { icon: '🗺️', title: 'Journey Trước Wireframe', text: 'Lập bản đồ cảm xúc đầy đủ trước khi dùng Figma phát hiện vấn đề hệ thống mà wireframe-first sẽ bỏ sót.' }
        ]
      }
    ]
  },
